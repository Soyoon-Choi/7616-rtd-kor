...
-   [Altibase 패키지 인스톨러](#altibase-package-installer)

-   [Altibase 제품 설치](#altibase-product-install)

-   [Altibase 클라이언트 제품 설치](#altibase-client-product-install)
...
(altibase-product-install)=
### Altibase 제품 설치

이 절에서는 유닉스에서 Altibase 제품을 설치하는 방법을 설명한다.

-   [설치 환경 사전 확인](check_the_environment_before_installation)

-   [패키지 인스톨러 다운로드](download_the_package_installer)

-   [설치 모드 설정](set_the_installation_mode)

-   [Altibase 패키지 인스톨러 시작](start_the_altibase_package_installer)

-   [설치 디렉터리 입력 및 설치 타입 선택](Enter_the_Installation_Directory_and_Select_the_Installation_Type)
    
-   [시스템 파라미터 확인](Check_System_Parameters)

-   [Altibase 프로퍼티 설정](Set_Altibase_Properties)

-   [설정한 프로퍼티 확인](Confirm_Altibase_Properties)

-   [Altibase 제품 설치](Install_the_Altibase_Product_1)

-   [Altibase 라이선스 입력](Register_or_Update_the_Altibase_License_Key)

-   [Altibase 빠른 설정 가이드 미리 보기](Preview_the_Altibase_Quick_Settings_Guide)

-   [설치 종료](Finish_Installation)

-   [설치 후 작업](Post_Installation_Tasks)
(check_the_environment_before_installation)=
#### 설치 환경 사전 확인
...
(download_the_package_installer)=
#### 패키지 인스톨러 다운로드 

Altibase 패키지 인스톨러는 [Altibase 고객지원서비스 포털](http://support.altibase.com/)을 방문하여 본인의 운영 체제에 적합한 인스톨러를 다운로드 받는다.

Altibase가 제공하는 운영 체제별 인스톨러는 "[패키지 인스톨러 다운로드](#package-installer-download)"를 참조하기 바란다.
(set_the_installation_mode)=
#### 설치 모드 설정
...
$ xhost +
```
(start_the_altibase_package_installer)=
...
```
(Enter_the_Installation_Directory_and_Select_the_Installation_Type)=
...
(Install_the_Altibase_Product_1)=
#### Altibase 제품 설치
...
```
(Register_or_Update_the_Altibase_License_Key)=
#### Altibase 라이선스 입력
...
```
(Preview_the_Altibase_Quick_Settings_Guide)=
...
- 설치단계에서 PSM 사용을 위한 프로퍼티를 입력하지 않은 경우, catproc.sql 을 실행하라.  
  ...
  ${ALTIBASE_HOME}/packages/catproc.sql
  ```
(altibase-client-product-install)=
