...
-   [Altibase 제품 설치](altibase_product_install)

-   [Altibase 클라이언트 제품 설치](altibase_client_product_install)
...
Altibase가 제공하는 운영 체제별 인스톨러는 "[패키지 인스톨러 다운로드](package_installer_download)"를 참조하기 바란다.
...
- 설치단계에서 PSM 사용을 위한 프로퍼티를 입력하지 않은 경우, catproc.sql 을 실행하라.  
  ...
  ${ALTIBASE_HOME}/packages/catproc.sql
  ```
(altibase_client_product_install)=
