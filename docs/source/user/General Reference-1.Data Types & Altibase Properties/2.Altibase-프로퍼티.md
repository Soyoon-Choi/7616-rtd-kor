...
DDL 복제 시 테이블 잠금 획득 실패(DDL_LOCK_TIMEOUT)와 같이 재시도 가능한 에러가 발생하면 이 프로퍼티에서 설정한 시간 동안 DDL 복제를 재시도한다.[REPLICATION_DDL_SYNC_TIMEOUT#1](#footnote-replication-ddl-sync-timeout-1)
...
:::{note}
:name: footnote-replication-ddl-sync-timeout-1
